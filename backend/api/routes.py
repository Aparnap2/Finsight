...
from backend.engine.assertion_pipeline import run_assertion_pipeline
from backend.engine.data_quality import assess_batch
from backend.engine.policy import evaluate_policy, evaluate_from_pipeline_result
from backend.engine.bridge_analysis import decompose_bridge
from backend.models.action import ActionItem, ActionDomain, ActionStatus, ActionImpact, create_action
...
def _run_full_pipeline(period: str, tenant_id: str) -> dict:
    ...
    if not sections:
        sections = []
        from backend.models.state import CommentarySection
